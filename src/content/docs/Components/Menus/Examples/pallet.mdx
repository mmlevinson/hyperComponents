...
import PalletGridMain from '../../../../../components/PalletGridMain.astro'
...
The second example creates the identical pallet but  with hover activation built with the [HoverDropdownMenu_B](/components/menus/hover_b) component outlined earlier.  Also, the orientation is vertical for small screens which is set by passing a string to the `orientation` prop.
...
<HoverDropdownMenu_B id='hover-activated-pallet' moveXY='-100,18' buttonStyles='colorPallet bg-gray-700' duration='180ms'>
	<img slot="leftIcon" src='/images/tailwind_icon.svg' alt='tailwind icon' class={tw.icon}/>
	<PalletGridMain orientation='vertical'/>
</HoverDropdownMenu_B>
</div>
