...
```js "ClickDropdownMenu_A" "PalletGrid" "colorPallet"
---
...
```js "HoverDropdownMenu_B" "PalletGrid" "colorPallet"
---
...
1. Dropdown color palette showing all basic Tailwind colors
2. Uses ClickDropdownMenu_A and HoverDropdownMenu_B hyperComponents
3. Both Landscape and Portrait variations (for mobile screens).
4. 
...
-Clicking a swatch dismisses the dropdown smoothly and then displays an alert showing the color name and hexadecimal value of the selection.
...
In the hover activated example, a Hyperscript behavior (ToggleDropdown) is installed into the  `<button>` from a `<script>` tag in the [HoverDropdownMenu_B](/components/menus/hover_b) component. This behavior adds tracking of the mouseenter and mouseleave system events in order to set dropdown visibility based on hover state. 
...
This hyperComponent utilizes either the  [ClickDropdownMenu_A](/components/menus/click_a) or [HoverDropdownMenu_B](/components/menus/hover_b) parent components presented earlier.
...
Each swatch is comprised of  a [PalletItem](#palletitem) component  which accepts a `twColor` and `hexCode` prop.   The `twColor` is the Tailwind color class and the `hexCode` is the equivalent hexadecimal value of the color.   
