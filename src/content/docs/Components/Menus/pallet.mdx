...
```
 ...
 <TabItem label='Dropdown'>
	<div id='example-2' class={tw.exampleContainer} >
		<DropDown id='dropdown-pallet' behavior="hover" moveXY='0,18' buttonStyles={tw.button} duration='180ms'>
			<img slot="leftIcon" src='/images/tailwind_icon.svg' alt='tailwind icon' class={tw.icon}/>
			<PalletGrid orientation='vertical' hs="on click send closeDropdown to the .colorPallet then settle then alert( 'You selected ' + my @title)"/>
		</DropDown>
	</div>
...
9. Parent components are  ClickDropdownMenu_A and HoverDropdownMenu_B hyperComponents whose default slot receives a [PalletGrid](#palletgrid) custom component.   
10. A class of `.colorPallet` is added to the `buttonStyles` prop of the parent component (Click/HoverDropdown) so that the Hyperscript associated with each swatch can send a custom event which triggers closure of the dropdown after a selection is made.
