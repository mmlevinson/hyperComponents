...
-Activation is by either hover or click

-Hyperscript code is located in a \<script\> tag with [Behaviors](https://hyperscript.org/docs/#behaviors) installed into elements inside the Component

The explanations are otherwise identical so you may simply [jump to the Code](#code) section.
:::
...
<HoverDropdownMenu_B id={'hover_dropdown'} caption="Hover Me" duration="300ms" moveXY="5, 5">
  <svg slot="endIcon" class={tw.icon}  fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
The example above is an Astro component comprised of a `<button>` and a `dropdown container` which wraps the `<ul>` containing a list of choices.  
...
Tapping any `<li>` emits a custom event attached to that item followed by dismissal of the dropdown.
...
The dropdown appearance/disappearance is smoothly animated by HyperScript with a timing function adjusted by the `duration` prop. 
...
Custom styles sent via [props](#code) are merged with the default styles and any conflicts resolved with a 'last wins' rule.

To improve readability, the Tailwind classes and Hyperscript code are extracted from the markup and encapsulated into Astro `tw` and `hs` local variables within the [Component Script](https://docs.astro.build/en/basics/astro-components/#the-component-script) which are then referenced in the html markup. 
...
You can use the `transition` keyword to explicitly designate a CSS selector, element, and transition effect within your Hyperscript code.  However, this technique is [blocking](https://hyperscript.org/docs/#transitions), and you will need to explicitly call the `settle` keyword after each `transition` statement which tells HS to wait briefly (20ms by default) to allow for swapping of all HS specific CSS classes used to implement the transition.

A simpler, but less flexible, technique is to apply a  CSS transition rule on the element being transitioned using an inline `<style>` tag.  This technique is not blocking and reduces the amount of code in the script itself.  This is the technique chosen for this hyperComponent. 

## Slots
### Default Slot

All menu items should be placed between the Component's opening and closing tags, thus positioning them into the default slot inside the Component's `<ul>`.

### Start Icon Slot

The `startIcon` slot is used to install any icon or image markup of your choice.   

If you also pass a `caption` prop, the `startIcon` slot will be displayed to the left of the caption text.

### End Icon Slot

The `endIcon` slot is used to install any icon or image markup of your choice which is then positioned after the caption text.  

In this example, a down-caret svg is used to provide a visual clue that interacting with the `<button>` will reveal additional choices.
...
You will have to also attach Tailwind styling classes to slot content  because the receiving Component  has no way to apply Tailwind classes to markup it does not know about yet. 

For example, icon sizes, padding, margins, and  colors may need to be specified. 

The code below illustrates Tailwind styling of a down-caret svg passed into the `endIcon` slot.

```html title="Tailwind Styling for Icon Slots"
  <svg slot="endIcon" class="inline-block size-6"  fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
		<path d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6z"/>
	</svg>
```
