...
The trigger button and dropdown are initially styled with Tailwind CSS utility classes but are fully customizable by passing additional classes through [props](#code).
...
Below is a summary of the component props which can optionally be used to configure the component from the markup where it is used.
...
The component's `<button>`, `<ul>` and `dropdown container` are initially styled with a default selection of Tailwind classes.  For accessibility, the trigger `<button>` also receives a focus ring when activated.
...
The full source code for this hyperComponent is visible in the various Tabs below.  Use the Everything tab to copy and paste the entire component into your Astro project.
