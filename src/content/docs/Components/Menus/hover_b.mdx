...
<HoverDropdownMenu_B id="hover_dropdown_id" caption="Hover Me" duration="300ms" moveXY="5, 5">
...
The trigger button and dropdown are initally styled with Tailwind CSS utility classes but are fully customizable by passing additional styling classes through [props](#code).
...
The duration of the height transition for the `dropdown container` can be specified by passing a string (in milliseconds) into the `duration` prop. 
...
The `leftIcon` slot is used to install any icon or image markup of your choic.   If you also pass a `caption` prop, the `leftIcon` will be displayed to the left of the caption text.
...
In the example code, a down-caret svg is used to present a visual clue that interacting with the `<button>` will reveal additional choices.

### Icon Styling

In Astro, you specify a named slot by including a `slot="name"` attribute in the parent element of the markup you are passing into the slot. 

You will also need to attach Tailwind styling classes to your slot markup because the receiving Component has no way to apply Tailwind classes to markup it does not know about yet. 
...
To use this hyperComponent in another document just import the file and use angle brackets (with a Capitalized name) to create a custom html tag as shown in the example below.

```html title="UsingADropdown.astro"
---
import DropdownMenu from './components/DropdownMenu.astro'
const tw = {
	menuItem: 'p-3 hover:bg-indigo-600 hover:text-white'
}
const hs = "on click call alert('You selected ' + my innerHTML) "
---

<DropdownMenu id="hover_dropdown_id" caption="Hover Me" duration="300ms" moveXY="-5, 5">
   <li class={tw.menuItem} script={hs}>Choice 1</li>
   <li class={tw.menuItem} script={hs}>Choice 2</li>
   <li class={tw.menuItem} script={hs}>Choice 3</li>
   <li class={tw.menuItem} script="on click send closeDropdown to #{'hover_dropdown_id'} then settle then go to url '/' ">Go Home</li>
