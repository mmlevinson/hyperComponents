...
import ClickDropdown_A from '../../../../components/ClickDropdown_A.astro';
...
<ClickDropdown_A id="click-dropdown-id" caption="Click Me" duration="300ms" moveXY="5, 5">
...
---
<ClickDropdown_A id="click-dropdown-id" caption="Click Me" duration="300ms" moveXY="10, 5">
  <svg slot="rightIcon" class={tw.icon}  fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
		<path d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6z"/>
	</svg>
   <li class={tw.menuItem} script={hs}>Choice 1</li>
...
```html title="Tailwind Styling for Icon Slots" "slot=\"rightIcon\""
		...
		const hs = {
			toggleDropdown: `
				...
				def resetPosition
					...
					set leftOffset to the offset[0] as Int
					set bottomOffset to the offset[1] as Int
					measure me then put the result.bounds into myBounds
					set the *left of the #{:dropdownContainer} to (myBounds.left + leftOffset + window.pageXOffset) px
					set the *top of the #{:dropdownContainer} to (myBounds.bottom  + bottomOffset + window.pageYOffset) px
				end  -- function to reposition dropdown by x,y pixels
				
				on openDropdown from me
					resetPosition()
					set the *height of the #{:dropdownContainer} to (calcHeight() + 'px') then wait 20ms then
...
```js title="UsingADropdown.astro" "ClickDropdown_A"
---
import ClickDropdown_A from '/components/ClickDropdown_A.astro';
...
---
<ClickDropdown_A id="click_dropdown_id" caption="Hover Me" duration="300ms" moveXY="10, 5">
  <svg slot="rightIcon" class={tw.icon}  fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
		<path d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6z"/>
	</svg>
   <li class={tw.menuItem} script={hs}>Choice 1</li>
...
Your custom scripts should always  `send closeDropdown to #{'click_dropdown_id'} then settle` to dismiss the dropdown gracefully before proceeding with any further actions (as in the 'Go Home' choice above).
