...
The trigger button and dropdown are initally styled with Tailwind CSS utility classes but are both fully customizable using [props](#props).
...
To reduce verbosity and improve readability the Tailwind classes are extracted from the markup and encapsulated into a local `tw` variable in the Astro component script and then interpolated into the template.
