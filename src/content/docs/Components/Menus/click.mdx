...
The example above is an Astro component comprised of a button and a dropdown container which wraps an unordered list displayed as a flexbox in column orientation.  The unordered list is populated by passing a list of items into the component's default slot.
...
```
The dropdown container is hidden by setting it's height to 0 and the opacity of the `<ul>` to 0.  The button listens for a click event which triggers the dropdown to smoothly appear by transitioning to it's full height and the `<ul>`'s opacity to 1 over a specified duration. 

Tapping any item in the dropdown emits a custom event attached to that item followed by dismissal of the dropdown.
