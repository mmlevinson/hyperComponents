...
```
The dropdown is  hidden by setting the initial height of it's container to 0.   The trigger button listens for a click event which causes the dropdown  to smoothly appear by transitioning the height and opacity of it's container over a specified duration. 

Tapping any item in the dropdown triggers a custom event to be fired followed by dismissal of the dropdown.

## Features

The trigger button and dropdown are initally styled with Tailwind CSS utility classes but are both fully customizable by passing additional styling classes through [props](#code).

The dropdown appearance and disappearance are smoothly animated by HyperScript. The dropdown is dismissed by:
...
Tapping any menu item triggers a custom event written in HyperScript
...
Each DropdownMenu component requires a unique id.  This is used to ensure that the dropdowns do not interact with each other.

### Caption

The `caption` prop is the text that will be displayed in the trigger button.  

### Button Styles

The `buttonStyles` prop allows you to pass additional Tailwind classes to the trigger button.  This is useful if you want to change the hover behavior, change the  background or text color, apply a dropshadow, etc.

### List Styles

The `listStyles` prop allows you to pass additional Tailwind classes to the dropdown list.  Any styles you pass in this prop apply to the parent `<ul/>` that wraps all menu items.
...
If the dropdown needs to be moved relative to the trigger button, you can pass a string with two comma separated values into the moveXY prop.  The first value is the horizontal shift and the second is the vertical shift.  For example, to move the dropdown 5 pixels to the right and 5 pixels down from the top left corner of the screen, you would pass `-5,5`.

### Transition Duration

The transition duration for the dropdown height can be customized by passing a string of milliseconds into the duration prop.  For example, to set the transition to 100 milliseconds, you would pass `100ms`.

## Styling

Tailwind classes are used for the default styling of component, including the trigger button and dropdown container. For accesssibility, the trigger button also receives a focus ring when activated.

To improve readability, the Tailwind classes and Hyperscripts are extracted from the markup and encapsulated into Astro `tw` and `hs` variables which are then referenced in the html markup.
...
To import the entire Astro component into your project, click the `Copy` button on the Everything tab above.  Paste all the code into a new Astro file.  In this example, I chose the file name of `DropdownMenu.astro`

To use this Component in any part of your Astro project, import the file and embed the Component into your markup within angle brackets (with capitalized tag name).
...
To install the menu items into the dropdown container just include multiple list items or other discrete elements between the parent tags.

Elements passed into the default slot can be of any makeup, and customized with any styling or action scripts.   
