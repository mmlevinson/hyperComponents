...
description: Active Search with HTMX with Highlighted Matches
tableOfContents: 
  minHeadingLevel: 2
  maxHeadingLevel: 5
---
...
export const tw = {
		...
		ol: 'mx-auto font-normal text-lg lg:text-xl '	
		},
...
<div id="introduction" class="mx-auto pb-4">
	<ol class={tw.introduction.ol + 'ml-4  p-4'}>
		<li>Click  any `Famous Speech` in the table below</li>
		<li>Use the  `Search box` to enter substrings to match</li>
		<li>Use the `(left caret) dropdown` to restrict search</li>
		<li>Click the `Tailwind dropdown` to change the hilite color</li>
		<li>Start over with the `Reset` button</li>
	</ol>
</div>
</div>
...
export const defaultSpeechTitle = 'Click on any of the famous orations listed above.'
export const defaultSpeechTranscript = 'The full transcript will be displayed here. Use the Search box to dynamically highlight any part of the transcript'
...
export const hs = {
	resetButton:`
		...
		on click 
			put the "${defaultSpeechTitle}" into the #{'speech-title'} then
			put the "${defaultSpeechTranscript}" into the #{'speech-transcript'} then 
			go to the #{'all-speeches'} smoothly
		end`,
	table: `								
		on reset
			remove .{^rowHiliteColor} from <tr/>
			set ^rowHiliteColor to the @bgColor of the #{'search-container'}  -- refresh if it has changes
		end `,
	colorPallet: `
		on click 
...
<div class={tw.speech.wrapper}>
	...
	<div id="speech-transcript" class={tw.speech.transcript}></div>
</div>
...
2. A  `target` prop which accepts a CSS selector that is used to update the 	`hx-target` attribute.   The `target` prop identifies the DOM element which will be updated with the returned hypertext.
...
11.  Styling of the `<input>` and dropdown menus is fully customizable using Tailwind utility classes
...
This hyperComponent is a prototype for [hypermedia exchanges](https://htmx.org/essays/hypermedia-apis-vs-data-apis/) because it illustrates a front-end coupled to backend routes that respond by returning only hypertext.

All functionality is powered by [htmx](https://htmx.org) and [Hyperscript](https://hyperscript.org).  There are no JavaScript imports or `<script>` tags anywhere in this component. This component is a port of the [Active Search example from the htmx documentation](http://htmx.org/examples/active-search/) but now including:

1. **live highlighting of matched substrings**
2. User selectable hilight color analogous to using a felt tipped  marker on a printed document.  
3. User selectable search filters.


 Substrings entered by the user is matched against the full speech transcript using Regular Expressions.  
 
 Successful matches are surrounded with `<mark>` tags styled with the user's choice of Tailwind bg-color-* utility class. The server then returns the entire transcript as HTML annotated with `<mark>` elements. 
...
The [code](#code) for the `Searchbox.astro` component is shown below.   In order for this component to function as intended, the developer must also provide backend code to process the request and return HTML as discussed in the [API Routes](#api-routes) section below. 

Dropdown menus within the Searchbox control are implemented using the [ClickDropdownMenu_A](/components/menus/click_a/) component.   A search-options dropdown appears to the left of the `<input/>` indicated by a down-caret icon.  Multiple `<li>` elements are passed into the default slot of this [ClickDropdownMenu_A](/components/menus/click_a/) which provide the options for modifying the RegEx match criteria used by the backend.   The textContent of the selected `<li>` is stored in a hidden field.  

Another [ClickDropdownMenu_A](/components/menus/click_a/) instance is used to present a pallet of Tailwind colors allowing the user to change the color of the highlighted matched substrings.   A [PalletGrid](/components/menus/examples/pallet/#palletgrid) component is passed to the default slot of this [ClickDropdownMenu_A](/components/menus/click_a/) to display the grid of swatches.   Hyperscript is used to store the user's selection into the pallet's 'bgColor' attribute and also update another  hidden `<input>` so the highlight color can be transported to the backend with each request.
...
```js
interface Props {
		id?:string  //unique identifier used by Hyperscript 
		placeholder?:string  //search box hint 
		searchPath:string //backend api route for RegEx matching
		target:string //htmx-compatible CSS selector for the hx-target
		swapStrategy:string, //htmx swap attribute ('innerHTML') 
		height?:string //TW class for setting search box height
}

//There is no width prop.   
//To adjust the width, wrap this hyperComponent in a container and then adjust the container's width. 
...
The magnifying glass icon to the right of the `<input>` is actually the default content of the `rightIcon` slot.    Most search controls display a magnifying glass icon as a symbol of the control's function.   In this example, the icon is inert (does not respond to clicks).   
...
The value of this attribute is picked up by the Hyperscript code and used to dynamically change the background color of the entire search box when the user picks a swatch from the color pallet dropdown. 

In addition, this color is sent to the backend along with the searcj request in order to provide styling for the `<mark>` tags which bracket the matched substrings in the response HTML.

In this manner the `data-hilight-color`  is used as a local cache for Hyperscript.
...
Here is the folder/file structure used:  
...
The tab panel below displays the entire source code for each backend route.

##### famousSpeeches.json

The `famousSpeeches.json` file contains the raw data for each speech. There is too much text to show all the data here, so one sample speech object is included to illustrate the data structure. All other speech entries use the same object structure.
...
A list of speeches is loaded into a `<table>` using the `index.js` route.  The raw data is looped using a `.map()` function returning a single `<tr>` for each Speech containing the speaker's name, title of the speech, and date.   

Notice that Hyperscript is also added to each `<tr>` so that a click on the row loads the individual transcript into a target `<div>`.
...
The `search-criteria` field contains the filter to be applied to the regular expression, such as 'Whole Word' or 'Case Sensitive', etc.  The value of this field is set when  the user chooses  an option from the dropdown at the left of the search `<input>`.
...
The remaining code in `match.js` uses a regular expression to match substrings in the speech transcript and then wrap each match with `<mark>` tags containing the specified Tailwind bg-color* class.
...
To visualize how the Searchbox component is used in a real-world situation, a mini-project is included in the [Examples](#examples) section above. 

There are many use cases for active search and even a highlighted active search.    In fact, too many use cases to cover here.  The bottom line is that most websites include a Search control somewhere in their header area, identified to the user by a magnifying glass icon.
...
For instructional purposes, the source code for implementing the Famous Speeches mini-project is included below in full detail.   Notice where and how the Searchbox control integrates with the other DOM elements to create the entire experience.

If you copy this code into your own project, you will have to adjust the import file paths to match your folder structure.

Also notice how extracting and coalescing the Tailwind classes into a local Astro `tw` constant unclutters the HTML markup.  In this example, the `tw` constant is an object with nested objects that permit clear and unambiguous organization for the styling code which enhances both readability and maintainability.
...
const defaultSpeechTitle = 'Click on any of the famous orations listed above.'
const defaultSpeechTranscript = 'The full transcript will be displayed here. Use the Search box to dynamically highlight any part of the transcript'
