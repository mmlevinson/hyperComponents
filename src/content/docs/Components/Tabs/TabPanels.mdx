...
Switching Tabs is performed exclusively by [HyperScript](https://hyperscript.org).   All styling is provided by [Tailwind](https://tailwindcss.com) utility classes. Developers can customize the styling of active and inactive tabs using Tailwind and these utility classes will then be utilized by the Hyperscript code.
...
2.  Content is loaded from custom API route using [htmx](https://htmx.org)
...
## Components
   ...
   All the  source code you need to implement this component in your project is presented in various tabs of the [Code](#code) section below.		
		...
		Included in the TabPanel component is a `tw constant` encapsulating all the base styles for both the container, the  panel `<output/>` element, and the individual [TabPanelItems](#tabpanelitem) displayed as tabs  at the top of the control.

		Exporting the tw constant allows for sharing key styles between the Hyperscript code within the container and the TabPanelItem instances themselves.  In order to switch the appearance of the tabs when activated, the Taildind classes  must be available to both the container which harbors the Hyperscript code for switching tabs and the individual tab components themselves.  This is done by simply exporting a tw constant from the TabPanel component and then importing it into the TabPanelItem component where some of the utility classes are grabbed to apply default styling to the tabs themselves.
   ...
   One of the power features of htmx is attribute inheritance.   The majority of attributes are also available within children of the parent component where those attributes are applied.   This spreads functionality to the children without code duplication.

	 Placing the following attributes on the container `div` of the parent TabPanel takes advantage of inheritance by providing the same attributes to the children TabPanelItems.
