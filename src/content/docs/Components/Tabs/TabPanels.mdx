...
import TabPanelItem from '../../../../components/tabs/TabPanelItem.astro'
	...
	<div class="p-4">
		<TabPanel defaultTabId="0">
			<TabPanelItem id="0" caption="Hypermedia" path="/api/readmore/0" trigger="load or click" />
...
In the example above , a row of classic semi-rounded Tabs is presented above a container where content is dynamically loaded according to the topic displayed in the currently selected Tab.  

This example  uses [Hyperscript](https://hyperscript.org) for UI and [Tailwind](https://tailwindcss.com) for styling.

There are two Astro components used to implement this control: 

1. [TabPanel](#tabpanel)  which acts as a parent container, and  
2. Multiple [TabPanelItem](#tabpanelitem)s inserted into the TabPanel default slot. 

Here is the code used to implement this example:

```astro title="Example.astro" "TabPanel" "TabPanelItem"
...
1. A top row of Tabs where click events switch content in the output element.
2. Smooth transitions during content swapping.  
3. Custom styling using [Tailwind](https://tailwindcss.com)
...
The parent container for this control is the `TabPanel` component.
#### Props
	{/* Each component, with code block showing the props interface */}
			
```js title="TabPanel.astro"
interface Props {
	defaultTabId?:string  //unique identifier of the Tab to be loaded as a the default
	maxHeight?:string    //TW class for maximum output height (ex: max-h-[35rem])
}
...
```
		...
		The Hyperscript code in the parent TabPanel component grabs this identifier and resolves a reference to the matching [TabPanelItem](#tabpanelitem).  That reference is then sent as a custom event argument so one tab is activated on page load.
		...
		This prop sets the maximum height of the `<output> ` element where the content is displayed.   This should be an appropriate Tailwind `max-h` value and can include any extemporaneous value in brackets.   
		...
		Another advantage of this strategy is maintainability. It is easier to update styling when everything is gathered in one place while staying close to the markup where it is used (as per [Locality of Behavior](https://htmx.org/essays/locality-of-behavior/)).
		...
		The `output` classes provide styling for `<output>` element where the content is displayed.  You can, of course, apply any styles of your choice to the markup returned by your custom endpoints.
    ...
    The Hyperscript is aware of the `tw` properties and adds/removes these classes by referencing the same constants inside the `hs` script string. If you need more custom behavior for the individual tabs, add matching classes to both `tw` and the `activateTab` logic shown in the [scripts tab](#code) below.
		...
		The `init` handler sets an element-scoped `:loadCount` value that is used as a guard against duplicate `load` events in this setup.

		##### load

		On load, the handler uses a guard clause: `if :loadCount is greater than 1 then exit end`. If the handler continues, it sends `activateTabById(tabId:'${defaultTabId}')` to the parent container, then increments `:loadCount`.


		##### activateTabById(tabId)

		This event accepts a unique tab id string, resolves it with `#{tabId}`, and if the element exists it forwards to `activateTab(clickedTab:requestedTab)`. This is the programmatic entry point for switching tabs from outside the component.

		##### activateTab(clickedTab)

		This event resets active/inactive utility classes across all tabs and then loops each tab to apply active classes only to the clicked tab, and inactive classes to all other tabs. The identity check uses `if tab is really clickedTab`.

		The two `remove ... from the <.tab-item/> in me` lines use Hyperscript's collection handling ("flat-mapping") so one command can clear classes from every tab in the panel.

		Because Tailwind class precedence can leave conflicting utility classes on one element, this implementation intentionally does not rely on a single `take` statement for all visual states. It removes both active and inactive class groups first, then reapplies the correct set per tab.
   ...
   One of the power features of htmx is [attribute inheritance](https://htmx.org/docs/#inheritance).   The majority of attributes are  available within children of the parent component where those attributes are applied.   This spreads functionality to the children without code duplication.
	 ...
	 Placing the following attributes on the container `div` of the parent [TabPanel](#tabpanel) takes advantage of inheritance by providing the same attributes to all child [TabPanelItems](#tabpanelitem).
		...
		The swap strategy is simple.  The response is swapped into the innerHTML of the target (below) with  `transition:true` signalling htmx to apply the ViewTransitions API for a seamless fade effect between outgoing and incoming content.
	 ...
	 In the scripts tab below notice the use of back-ticks for the `hs` constant.  By extracting the script into a local constant and then using back-ticks with string interpolation, it is possible to embed individual Tailwind utility classes and other props directly into the Hyperscript code while at the same time allowing the markup to utilize these same values.

		You would not be able to  cross-purpose the styling with the HS code if you kept the Tailwind classes inside the `class` attribute of the DOM elements themselves.
...
Since only one tab can be activated at a time, be careful not to overload the trigger props so they interfere with each other.
	...
	The [TabPanel](#tabpanel) container uses most of these Tailwind classes to switch the appearance of active and inactive tabs.

	At the same time, exporting these styles and then importing them into the child component (TabPanelItem) shares the styling in such a manner that changes made in the parent are propagated to the children.
  ...
  You could just destructure the properties of the `tw` constant that are used in the child component, but the coding syntax used above is just plain simple and stable.
	...
	##### activateTab(clickedTab:me)

	This custom event is emitted when the `<button>` element is clicked. The target of this event is the parent `div.tab-panel`, which listens for this event and switches the active tab to the element passed in `clickedTab` parameter.
...
The default trigger event (`click`) is set in the TabPanelItem `trigger` prop and can be overridden in each child [TabPanelItem](#tabpanelitem) by passing different event(s), which are then referenced by the `hx-trigger` attribute in the markup.
	...
	To utilize this component in your project, copy and paste the contents of the Everything panel into an `.astro` file in your src folder.
	...
	<Tabs>
		...
		```js
		const hs = `
		on click send activateTab(clickedTab:me) to the closest <.tab-panel/>
		`
		```
		</TabItem>
...
##  Usage
	...
	You can also switch tabs programmatically by sending `activateTabById` to the TabPanel container:
