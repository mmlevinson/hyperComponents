...
In the example below, classic rounded tabs utilize a `<button>` which uses htmx to send an AJAX request to a custom API endpoint and then retrieves a hypermedia response which is displayed in a fixed height container below the row of tabs.  

Tab switching is performed exclusively by HyperScript.   

All styling is from Tailwind.

There are two Astro components used to provide this control, a [TabPanel](#tabpanel) component which is a parent container that  wraps multiple [TabPanelItem](#tabpanelitem) components.   

You can customize the active and inactive tabs using Tailwind and these utility classes will be picked up and implemented by the Hyperscript code.
		 
## Example
		 
export const tw = {
		exampleContainer: 'not-content w-full w-max-lg text-center mx-auto bg-gray-300 dark:bg-gray-800 rounded-xl my-6',
		exampleTitle: 'bg-gray-800 dark:bg-sky-900 text-gray-300 dark:text-zinc-200 font-semibold py-2 mb-4 rounded-t-xl',
