...
## Components
   ...
   All the  source code you need to implement this component in your project is presented in various tabs of the [Code](#code) section below.		
    ...
    The Hyperscript is aware of the `tw` properties and adds/removes these classes by referencing the same constants inside the `hs` script string. If you need more custom behavior for the individual tabs, add matching classes to both `tw` and the `activateTab` logic shown in the [scripts tab](#code) below.
		...
		On load, the handler resolves `defaultTabId` to an element reference and sends `activateTab(clickedTab:defaultTab)` to the parent container. It only performs that activation while `:loadCount` is less than `2`, then increments the counter.
			...
			const hs = `
				...
				on load
					if :loadCount is less than 2 then 
						...
						send activateTab(clickedTab:defaultTab) to me
					end
					increment :loadCount
				end -- load handler
