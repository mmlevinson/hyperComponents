...
## Description
	...
	The basic source code to implement the examples  is shown below.   The [code](#code) sections below contains the props, scripts, and styling of both Astro components.   In order to use this ReadMore example, you will need to set up analagous [custom API endpoints](https://docs.astro.build/en/guides/endpoints/) and return appropriate html responses but the example code should help you get started.
...
#### Events
	...
	The ReadMore effect is gained by setting the initial height of the .body to zero.   In addition to TW utility classes a `body` class is added to the div containing the lazy-loaded additional content.   The initial height is set to zero.   This strategy keeps the div within the DOM so that the script can access the offsetHeight property of each child.    

	The script recalculates the final height based on the offsetHeight of each child within the div.body.   

	A style tag with transition timing function is added to the div.body.    When the script sets a new value to the height property of the div.body, this transition is triggered behind the scenes.

  You can set the default duration of the transition using the duration prop.    Using string interpolation, the value of this prop can be accessed by the script.
