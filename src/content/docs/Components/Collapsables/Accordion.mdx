...
This prop takes a string which sets the maximum height of the accordion body.  The default is `300px`.  

You can pass any value but must include the CSS units for the script to apply correctly.  This technique allows using scalable values (such as rem or %) for limiting the body content height. 

 Valid examples would be something like :
...
To make component props available to the script, the HS code is placed into a back-tick delimited JS string and stored in a local constant in the Astro [Component Script](https://docs.astro.build/en/basics/astro-components/#the-component-script) (i.e. frontmatter).  This allows prop values to be inserted into the script using simple string interpolation.  

```astro  "${maxHeight}" "${openDuration}"
	`transition the #{:accordionBody}'s *height to ${maxHeight}  using 'all ${openDuration} ease-in' `
```

The local `hs` constant storing the code block is then referenced in the markup as would any variable in the component frontmatter using curly braces as shown below.
...
See the [Code](#code) section below for all the scripts  and how they are populated with the values of component props using this technique.
